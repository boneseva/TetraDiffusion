...
def load_gt_data(gt_dir, num_points=2048, max_gt=None):
    ...
    if max_gt and total_gt_count > max_gt:
        np.random.seed(42)
        indices = np.random.choice(total_gt_count, max_gt, replace=False)
        gt_files = [gt_files[i] for i in sorted(indices)]
        log(f"  [GT] Capped ground truth loading to {max_gt} samples (out of {total_gt_count} total).")
